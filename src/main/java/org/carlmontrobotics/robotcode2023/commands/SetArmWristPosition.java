...
public class SetArmWristPosition extends SequentialCommandGroup {
    ...
    public SetArmWristPosition(double armPos, double wristPos, Arm arm) {
        super();
        // TODO: Test it!
        // if arm is lower than MID when moving and if goal is lower than this by ANY
        // AMOUNT, then fold the wrist first
        // need separate flipped case for front too since values are flipped when arm
        // passes through bellypan
        /*
        if ((arm.isFront && (arm.getArmPos() < arm.getArmGoal(GoalPos.MID)) && (arm.getArmPos() > armPos)) ||
                (!arm.isFront && (arm.getArmPos() > arm.getArmGoal(GoalPos.MID)) && (arm.getArmPos() < armPos))) {
            // but have special case where arm is in front and wrist would collide, so we
            // have to move arm pos to mid before folding
            ...
            super.addCommands(
                    ...
                    new WaitUntilCommand(arm.armPID::atSetpoint),
                    new InstantCommand(() -> arm.setWristTarget(wristPos)), // assume wristPos isn't gonna collide with
                                                                            // drivetrain because that'd be a stupid
                                                                            // thing for people to do and I don't wanna
            ...
            super.addCommands(// no collision? move both at once for moar speed
                    ...
                    new WaitUntilCommand(() -> {
                        return (arm.wristPID.atSetpoint() && arm.armPID.atSetpoint());
                    }));
        }
        addRequirements(arm);
        */
