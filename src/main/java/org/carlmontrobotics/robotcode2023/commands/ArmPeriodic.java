...
public class ArmPeriodic extends CommandBase {
  ...
  public void execute() {
    ...
    armSubsystem.driveArm(speeds[0], 0);
    armSubsystem.driveWrist(speeds[1], 0);
    armSubsystem.setArmTarget(armSubsystem.getArmPos());
    armSubsystem.setWristTarget(armSubsystem.getWristPos());
  }

  public double[] getRequestedSpeeds() {double rawArm, rawWrist, deltaT;
    deltaT = .05;
    ...
    if (Math.abs(arm.getAsDouble()) <= Constants.OI.JOY_THRESH) rawArm = 0.0;
    else rawArm = MAX_FF_VEL * arm.getAsDouble();
    if (Math.abs(wrist.getAsDouble()) <= Constants.OI.JOY_THRESH) rawWrist = 0.0;
    else rawWrist = MAX_FF_VEL * wrist.getAsDouble();

    double targetAccelerationArm = (rawArm - currentArm)/deltaT;
    double targetAccelerationWrist = (rawWrist - currentWrist)/deltaT;
    if (targetAccelerationArm >= MAX_FF_ACCEL) {
      targetAccelerationArm *= MAX_FF_ACCEL/targetAccelerationArm;
    }
    if (targetAccelerationWrist >= MAX_FF_ACCEL) {
      targetAccelerationWrist *= MAX_FF_ACCEL/targetAccelerationWrist;
    }
